...
class DynamicDiscountStrategy:
    """Универсальная стратегия скидок — параметры читаются из Supabase."""
...
def get_strategy(service_id: int, company_id: int = COMPANY_ID) -> DynamicDiscountStrategy:
    """Читает стратегию из Supabase. Fallback на step если не найдена."""
...
async def _fetch_slots_for_date(
    ...
    async def fetch_one(svc):
        try:
            ...
            print(f"[SLOTS] timeout service {svc['id']}: {e}")
            return []
        strategy = get_strategy(svc["id"], company_id)
        items = []
        all_slots = slots_resp.get("data", [])
        valid = [s for s in all_slots if datetime.fromisoformat(s["datetime"]).timestamp() - now_ts >= gap_sec]
...
async def get_featured(date: str = Query(None)):
    """BentoGrid — топ слоты по стратегии. Приоритет premium, может включать завтра."""
    if not date:
        date = dt_module.today().isoformat()

    token = get_user_token()
    is_weekend = datetime.now().weekday() >= 5
    services = FEATURED_SERVICES_WEEKEND if is_weekend else FEATURED_SERVICES_WEEKDAY

    svc_prices = {
        s["id"]: s["price_min"]
        for s in supabase.table("services").select("id,price_min")
            .in_("id", [s["id"] for s in services]).execute().data
    }

    results = await _fetch_slots_for_date(date, services, svc_prices, token)

    # Fallback 1: нет premium → добавляем один premium из ближайших дней в начало
    ...
    # Fallback 2: совсем мало → добираем из следующего дня в конец
    if len(results) < 3:
        for delta in range(1, 8):
            next_date = (dt_module.fromisoformat(date) + timedelta(days=delta)).isoformat()
            extra = await _fetch_slots_for_date(next_date, services, svc_prices, token)
...
async def get_slots_stream(date: str = Query(None)):
    """AllSlots — ближайшие окошки строго по времени. Сегодня приоритет, завтра в конец."""
    if not date:
        date = dt_module.today().isoformat()
    ...
    # Добираем завтра в конец если мало
    if len(results) < 3:
    ...
    for s in results:
        ...
        if key not in seen:
            ...
            deduped.append(s)
    results=deduped
    results.sort(key=lambda x: x["minutes_to_slot"])
    results=results[:12]
...
async def get_strategies(company_id: int = Query(COMPANY_ID)):
    """Dashboard — список стратегий салона по категориям услуг."""
    res = supabase.table("service_strategies") \
        .select("*") \
        .eq("company_id", company_id) \
        .order("category") \
