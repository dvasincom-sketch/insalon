...
async function loadPayroll() {
  ...
  const data    = await fetchData('/analytics/payroll?months=12');
  if (!data || !data.payroll) return;

  // Заполняем селект месяцев из реальных данных
  ...
  if (sel) {
    ...
    const uniqueMonths = [...new Set(data.payroll.map(p => p.period_start.slice(0, 7)))].sort().reverse();
